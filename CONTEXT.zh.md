...
文档、实现和界面文案的统一说法。Navor 用 `.nav` 纯文本，在 Git 友好的**投资仓库**里描述**投资状态**。[English](CONTEXT.md)
...
| Account | 顶层配置层（sleeve）、市场边界、策略边界或现金边界。 |
...
- 指 `.nav` 文件夹时用**投资仓库**；指 `nav serve <workspace>` 参数时用**工作区**。
- 顶层配置层使用 `Account`（亦称 sleeve），不要使用 bucket 或 category。
