# @navor/cli

## 0.6.5

### Patch Changes

- Show realized and unrealized PnL as separate Reader summary metrics.
  - @navor/reader-ui@0.6.5
  - @navor/core@0.6.5
  - @navor/renderer@0.6.5

## 0.6.4

### Patch Changes

- Polish Reader UI consistency across views and fix table header styling.
  - @navor/core@0.6.4
  - @navor/renderer@0.6.4
  - @navor/reader-ui@0.6.4

## 0.6.3

### Patch Changes

- Show plan bodies and USDT budget caps in the Reader, render full action rules without truncation, and list knowledge, process, and plan records newest-first.
  - @navor/core@0.6.3
  - @navor/renderer@0.6.3
  - @navor/reader-ui@0.6.3

## 0.6.2

### Patch Changes

- Automatically refresh the Reader when a new PWA version becomes available.
- Updated dependencies
  - @navor/core@0.6.2
  - @navor/renderer@0.6.2
  - @navor/reader-ui@0.6.2

## 0.6.1

### Patch Changes

- Updated dependencies
  - @navor/reader-ui@0.6.1
  - @navor/core@0.6.1
  - @navor/renderer@0.6.1

## 0.6.0

### Patch Changes

- Updated dependencies
  - @navor/reader-ui@0.6.0
  - @navor/core@0.6.0
  - @navor/renderer@0.6.0

## 0.5.5

### Patch Changes

- Add the `nav check` command to validate a workspace and report parser and semantic diagnostics with a non-zero exit code, and fix Reader plan version comparison.
- Updated dependencies
  - @navor/reader-ui@0.5.5
  - @navor/core@0.5.5
  - @navor/renderer@0.5.5

## 0.5.4

### Patch Changes

- Refresh the Reader experience with updated terminology, focus handling, and scrolling behavior.
- Updated dependencies
  - @navor/core@0.5.4
  - @navor/renderer@0.5.4
  - @navor/reader-ui@0.5.4

## 0.5.3

### Patch Changes

- Updated dependencies [722becc]
  - @navor/reader-ui@0.5.3
  - @navor/core@0.5.3
  - @navor/renderer@0.5.3

## 0.5.2

### Patch Changes

- Updated dependencies [7636c92]
  - @navor/reader-ui@0.5.2
  - @navor/core@0.5.2
  - @navor/renderer@0.5.2

## 0.5.1

### Patch Changes

- Updated dependencies
  - @navor/reader-ui@0.5.1
  - @navor/core@0.5.1
  - @navor/renderer@0.5.1

## 0.5.0

### Patch Changes

- Updated dependencies
  - @navor/reader-ui@0.5.0
  - @navor/core@0.5.0
  - @navor/renderer@0.5.0

## 0.4.4

### Patch Changes

- Updated dependencies [1a1cbc0]
  - @navor/reader-ui@0.4.4
  - @navor/core@0.4.4
  - @navor/renderer@0.4.4

## 0.4.3

### Patch Changes

- Updated dependencies
  - @navor/renderer@0.4.3
  - @navor/reader-ui@0.4.3
  - @navor/core@0.4.3

## 0.4.2

### Patch Changes

- Updated dependencies
  - @navor/core@0.4.2
  - @navor/reader-ui@0.4.2
  - @navor/renderer@0.4.2

## 0.4.1

### Patch Changes

- Updated dependencies
  - @navor/core@0.4.1
  - @navor/renderer@0.4.1
  - @navor/reader-ui@0.4.1

## 0.4.0

### Patch Changes

- Updated dependencies
  - @navor/core@0.4.0
  - @navor/reader-ui@0.4.0
  - @navor/renderer@0.4.0

## 0.3.2

### Patch Changes

- Remove post-publish registry distribution verification from the release workflow. CI already covers pack and install smoke tests; waiting on npm registry propagation was fragile and redundant.
  - @navor/core@0.3.2
  - @navor/renderer@0.3.2
  - @navor/reader-ui@0.3.2

## 0.3.1

### Patch Changes

- Republish the CLI release after fixing the release workflow's registry distribution verification.
  - @navor/core@0.3.1
  - @navor/renderer@0.3.1
  - @navor/reader-ui@0.3.1

## 0.3.0

### Minor Changes

- 448da76: Improve Yahoo quote fetching with batching, caching, and 429 retry handling. Add dynamic Reader document titles and expand `nav format` support. Ship the VS Code / Cursor extension with syntax highlighting and format-on-save. Rewrite documentation with a manifesto, layered docs, and polished Chinese copy.

### Patch Changes

- @navor/core@0.3.0
- @navor/renderer@0.3.0
- @navor/reader-ui@0.3.0

## 0.2.0

### Minor Changes

- b47f7ff: Publish the Navor command-line interface as `@navor/cli` and provide the `nav` command. The unscoped `navor` package is no longer published.

### Patch Changes

- @navor/renderer@0.2.0
- @navor/reader-ui@0.2.0
