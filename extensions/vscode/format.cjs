...
function collectTopLevelItems(lines) {
  const items = []
  let index = 0
...
function formatDirectiveHeader(line) {
...
function formatDirectiveBlock(item) {
  const lines = [item.header]
  const postings = item.children.filter((child) => child.kind === 'posting')
...
function formatPostingLines(postings) {
...
function formatSinglePosting(posting) {
  const price =
    posting.priceAmount && posting.priceCurrency
      ? ` @ ${posting.priceAmount} ${posting.priceCurrency}`
      : ''
